...
function TagSize({
  ...
  const variantStyle = {
    lightgray: 'bg-bg-muted',
    gray: 'bg-white border border-border',
    blue: 'bg-info text-white',
  };

  const textColor =
    variant === 'blue' ? '' : size === 'L' ? 'text-text-muted' : 'text-text';

  return (
    <span
      className={`inline-flex items-center font-['Spoqa_Han_Sans_Neo'] cursor-pointer ${sizeStyle[size]} ${variantStyle[variant]} ${textColor} ${className}`}
      {...rest}
    >
      {children}
    </span>
