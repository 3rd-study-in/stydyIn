...
const SideNavButton = ({
  children,
  variant = 'lightgray', // blue, lightgray, white
  icon,
  onClick,
  disabled = false,
  className = '',
  ...
  const baseStyles =
    'w-[170px] h-[30px] px-4 inline-flex items-center font-sans font-medium text-base tracking-tight transition-all duration-200 rounded-md focus:outline-none disabled:cursor-not-allowed overflow-hidden cursor-pointer';

  // 3. 테마 변수 기반 색상 스타일
  const variantStyles = {
    // 배경이 진한 blue일 때 글자가 얇아 보이지 않도록 antialiased 추가
    blue: 'bg-primary text-white antialiased',
    lightgray: 'bg-white text-text hover:bg-bg-muted',
    white: 'bg-white text-text hover:bg-bg-muted',
