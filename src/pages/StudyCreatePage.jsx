...
const inputCls =
  'h-[40px] px-3 border border-border rounded-md text-sm text-text outline-none focus:border-2 focus:border-info';
const textareaCls =
  'w-full border border-border rounded-md p-4 text-base text-text placeholder:text-text-disabled outline-none resize-none focus:border-2 focus:border-info';
const chipBase =
...
function StudyCreatePage() {
  ...
  const [userRegion, setUserRegion] = useState(null); // { id, location }
  ...
  const handleThumbnailChange = async (e) => {
    ...
    if (result.ok) setField('thumbnail')(result.imageUrl);
  };
    ...
    <div>
      ...
      <div className="max-w-[1190px] mx-auto px-5 py-10">
        ...
        <div className="flex h-[390px] w-full overflow-hidden rounded-lg border border-border bg-bg shadow-sm mb-10">
          {/* 이미지 영역 (클릭 시 업로드) */}
          <div
            className="h-full w-[390px] shrink-0 border-r border-border bg-bg-muted flex items-center justify-center cursor-pointer relative overflow-hidden"
          ...
          {/* 우측 입력 영역 */}
          <div className="flex flex-1 flex-col justify-center gap-6 p-[30px]">
            {/* 제목 */}
            <div>
            ...
            <div>
                ...
                <label className="flex items-center gap-2 cursor-pointer">
                  <input
                    ...
                    onChange={() => {
                      ...
                      if (userRegion)
                        setField('study_location')({ id: userRegion.id });
                    }}
              ...
              {form.is_offline && (
                <p className="mt-2 text-sm text-info flex items-center gap-1">
                  <span>📍</span>
                  <span className="font-medium">
                    {userRegion?.location ?? '지역 정보 없음'}
                  </span>
